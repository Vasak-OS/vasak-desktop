...
<template>
  <AppWindowFrame container-class="w-screen rounded-t-[var(--radius-window)] flex flex-col overflow-hidden">
      <div class="h-12 border-b border-primary flex items-center px-4 bg-ui-bg/80 backdrop-blur-sm shrink-0">
        <div class="flex gap-2 mr-4">
          <button @click="navigateUp" aria-label="up" class="hover:text-primary transition-colors text-lg">
            ↑
          </button>
        </div>
        <div class="flex-1 bg-ui-bg/80 rounded-corner px-3 py-1 text-sm font-medium opacity-80 backdrop-blur-md truncate">
          {{ currentPath }}
        </div>
        <div class="ml-4 flex gap-2 items-center">
          <label class="flex items-center gap-2 text-xs font-medium cursor-pointer select-none">
            <input type="checkbox" :checked="showHidden" @change="toggleHiddenFiles"
              class="rounded-corner bg-ui-bg/80 focus:ring-primary text-primary" />
            Show Hidden
          </label>
      ...
      <div class="flex-1 flex min-h-0">
        <!-- Sidebar -->
        <div class="w-48 p-2 border-r border-primary flex flex-col gap-1 bg-ui-bg/80">
          <button v-for="item in sidebarItems" :key="item.name" @click="handleSidebarClick(item)"
            class="flex items-center gap-3 px-3 py-2 rounded-corner hover:bg-ui-bg/80 transition-colors text-sm font-medium text-left">
        ...
        <div class="flex-1 p-4 overflow-y-auto">
          ...
          <div v-else class="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-4">
            <div v-for="file in files" :key="file.name" @dblclick="handleItemClick(file)"
              ...
              <div
                class="w-16 h-16 flex items-center justify-center overflow-hidden rounded-corner bg-ui-bg/80 group-hover:scale-105 transition-transform duration-200">
                <!-- Image Preview -->
                <img v-if="
                  file.mimeType === 'image' &&
                  file.previewUrl &&
