...
<template>
  <WindowFrame class="rounded-t-window">
    <div
      class="h-[calc(100vh-32px)] w-screen rounded-b-window background flex flex-col overflow-hidden"
    >
      <div
        class="h-12 border-b border-vsk-primary flex items-center px-4 background backdrop-blur-sm shrink-0"
      >
        <div class="flex gap-2 mr-4">
          <button
            @click="navigateUp"
            class="hover:text-vsk-primary transition-colors text-lg"
          >
            ↑
          </button>
        </div>
        <div
          class="flex-1 background rounded-vsk px-3 py-1 text-sm font-medium opacity-80 backdrop-blur-md truncate"
        >
          {{ currentPath }}
        </div>
        <div class="ml-4 flex gap-2 items-center">
          ...
          <button
            class="p-1 hover:bg-vsk-primary rounded-vsk"
          >
            🔍
          </button>
        </div>
      ...
      <div class="flex-1 flex min-h-0">
        <!-- Sidebar -->
        <div
          class="w-48 p-2 border-r dark:border-vsk-primary flex flex-col gap-1 background"
        >
          <button
          ...
          >
            <img
              v-if="sidebarIcons[item.name]"
              :src="sidebarIcons[item.name]"
              alt="Icon"
              class="w-5 h-5"
            />
            <span v-else>📁</span>
        ...
        <div class="flex-1 p-4 overflow-y-auto">
          ...
          <div
            v-else-if="error"
            class="flex justify-center items-center h-full text-red-400"
          >
            {{ error }}
          </div>
          <div
