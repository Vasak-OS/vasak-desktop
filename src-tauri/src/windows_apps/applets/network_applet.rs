...
pub async fn create_applet_network_window(
    app: AppHandle,
    ...
    let window = WebviewWindowBuilder::new(
        &app,
        "applet_network",
        WebviewUrl::App("index.html#/applets/network".into()),
    )
    .title("Vasak network Applet")
    .decorations(false)
    .transparent(true)
    ...
    let applet_network_window = Arc::new(window);

    let complete_url = format!("{}/index.html#/applets/network", get_app_url());
    let url = Url::parse(&complete_url).expect("Failed to parse URL");
    let _ = applet_network_window.navigate(url);
    ...
    applet_network_window.set_position(Position::Physical(PhysicalPosition {
        x: center_x,
        y: center_y,
    }))?;

    applet_network_window.set_focus()?;

    set_window_properties(&applet_network_window);
