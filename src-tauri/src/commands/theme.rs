...
pub fn toggle_system_theme() -> Result<(), String> {
    log_info("Alternando tema del sistema");
    let output = Command::new("gsettings")
        .args(&["get", "org.gnome.desktop.interface", "color-scheme"])
        .output()
        .map_err(|e| {
            log_error(&format!("Error al obtener esquema de color actual: {}", e));
            format!("Error getting current color scheme: {}", e)
        })?;

    let current_scheme = String::from_utf8_lossy(&output.stdout).trim().to_string();
    log_info(&format!("Esquema de color actual: {}", current_scheme));

    // Cambiar entre esquemas de color y temas GTK
    if current_scheme == "'prefer-light'" {
        log_info("Cambiando a tema oscuro");
        ...
        log_info("Cambiando a tema claro");
        Command::new("gsettings")
            .args(&[
                "set",
                "org.gnome.desktop.interface",
    ...
    log_info("Tema del sistema cambiado correctamente");
    Ok(())
}
